...
/**
 * @description Función que permite consultar los documentos de compra
 * @creation David Villanueva 04/01/2020
 * @update
 */
exports.consultarCompraDocumento = async function (oFiltro) { 
    const oResponse = {};
    try {
        var oFiltroLista = {}; 
        oFiltroLista.where ={}; 
        ...
        const consultarListaResponse = await  compraDocumento.findAll(oFiltroLista); 
        if(consultarListaResponse.length > 0){
            ...
            oResponse.iCode     = 2;
            oResponse.sMessage  = 'No se encontro información de los documentos de compra'; 
